...
export class Color {
	...
	constructor( r: number | Color, g?: number, b?: number ) {
		...
		if ( ( g === undefined && b === undefined ) || typeof r === 'object' ) {
	...
	setStyle( style: string ) {
		...
		var m: RegExpExecArray | undefined | null;
		...
		if ( style && style.length > 0 ) {
			...
			// color keywords
			const hex: number | undefined = ( ColorKeywords as any )[ style ];
	...
	copySRGBToLinear = ( () => {
		...
		const copySRGBToLinear = ( color: Color ) => {
	...
	copyLinearToSRGB = ( () => {
		...
		const copyLinearToSRGB = ( color: Color ) => {
	...
	getHSL( target: HSL ) {
		...
		let hue: number | undefined, saturation: number | undefined;
	...
	lerpHSL = ( () => {
		...
		var hslA: HSL = { h: 0, s: 0, l: 0 };
		var hslB: HSL = { h: 0, s: 0, l: 0 };

		const lerpHSL = ( color: Color, alpha: number ) => {
